...
while IFS= read -r package; do
    echo '  DEB rm -rf ~/.cargo/registry/ ~/.cargo/.git/'
	...
	echo '    === DEBUG START ==='
	echo "${tmp}" | sed 's/^/    /'
	echo "${tmp}" | grep -E '^ ' | grep -oP '[a-zA-Z0-9_-]+ v[0-9.]+' | sort | uniq | sed 's/^/    /'
	echo '    ===  DEBUG END  ==='
	transitive_count=$(echo "${tmp}" | grep -E '^ ' | grep -oP '[a-zA-Z0-9_-]+ v[0-9.]+' | sort | uniq | wc -l)
